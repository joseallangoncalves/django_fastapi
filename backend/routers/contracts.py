...
from agent_skills.contract_extractor import skill_extrair_contrato
...
async def upload_contrato(
    ...
    # 2. Execução da Habilidade de IA (Groq Cloud)
    ...
    try:
        ...
        # Fallbacks em caso de formato incorreto da IA
    ...
    novo_contrato = models.Contrato(
        ...
        observacoes="" # Campo de observações inicialmente limpo
    )
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Erro ao persistir o contrato no banco de dados (provável número de contrato duplicado): {str(e)}"
        )
        
    # 5. Log de execução do Agente de IA na tabela de histórico
    historico_log = models.HistoricoAgente(
        usuario_id=user_id,
        habilidade="contract_extractor",
        input_prompt=f"Nome do arquivo: {file.filename}\nTamanho: {len(texto_bruto)} caracteres.",
        output_response=f"Contrato extraído: {novo_contrato.numero_contrato}\nContratante: {novo_contrato.contratante}\nContratado: {novo_contrato.contratado}\nValor: {novo_contrato.valor_total} {novo_contrato.moeda}",
